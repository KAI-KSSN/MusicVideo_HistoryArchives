...
export const videos:MusicVideo[]=(videosJson as MusicVideo[]).map(v=>{const seed=[...`${v.artist}${v.title}`].reduce((s,c)=>s+c.charCodeAt(0),0),[p,s]=palette[seed%palette.length];return{...v,slug:slug(v),primaryColor:v.primaryColor||p,secondaryColor:v.secondaryColor||s}});
